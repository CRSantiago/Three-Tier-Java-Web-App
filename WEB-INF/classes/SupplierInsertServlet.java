...
public class SupplierInsertServlet extends HttpServlet {
	...
	protected void doPost( HttpServletRequest request, HttpServletResponse response ) throws ServletException, IOException  {
		...
		String inBoundsname = request.getParameter("sname").trim();
		String inBoundStatus = request.getParameter("status").trim();
		...
		if(inBoundButton.equals("execute")) {
			...
			try {
				...
				String insertString = "insert into suppliers values (? ,? ,?,?);";
				PreparedStatement insertStatement = connection.prepareStatement(insertString);
				insertStatement.setString(1, inBoundsnum);
				insertStatement.setString(2, inBoundsname);
				insertStatement.setInt(3, Integer.parseInt(inBoundStatus));
				insertStatement.setString(4, inBoundCity);
				
				setNumRowsUpdated(insertStatement.executeUpdate());
				
				String confirmString = "New supplier record: (" + inBoundsnum + ", " + inBoundsname + ", " + inBoundStatus + ", " + inBoundCity + ") - successfully enetered into database.";
	...
	private void returnToClient(HttpServletRequest request, HttpServletResponse response, String returnMessage) throws ServletException, IOException {
		...
		HttpSession session = request.getSession();
		session.setAttribute("rowsUpdatedMessage", rowsUpdatedMessage); 
